...
fn compute_status(n_valid: i32, nights_since_update: i32) -> BaselineStatus {
    ...
    } else if n_valid < MIN_NIGHTS_SEED {
...
impl BaselineState {
    pub fn usable(&self) -> bool {
        self.n_valid >= MIN_NIGHTS_SEED
    }
    pub fn trusted(&self) -> bool {
        self.n_valid >= MIN_NIGHTS_TRUST
    }
}
...
pub fn update(state: Option<BaselineState>, value: Option<f64>, cfg: &MetricCfg) -> BaselineState {
    ...
    let Some(v) = value else {
        ...
        return BaselineState {
            ...
            status: compute_status(s.n_valid, m),
        };
    };
    ...
    if s.n_valid >= MIN_NIGHTS_SEED && !is_young {
        ...
        if dev > HARD_OUTLIER_K * s.spread {
            return BaselineState {
                ...
                n_valid: s.n_valid,
                nights_since_update: 0,
                status: compute_status(s.n_valid, 0),
            };
        }
    }
...
impl NightMetric {
    pub fn cfg(self) -> MetricCfg {
        match self {
            Self::Hrv => MetricCfg::hrv(),
            Self::RestingHr => MetricCfg::resting_hr(),
            Self::Resp => MetricCfg::resp(),
            Self::SkinTemp => MetricCfg::skin_temp(),
        }
    }
    fn value(self, n: &NightChannels) -> Option<f64> {
        match self {
            Self::Hrv => n.hrv_ms,
            Self::RestingHr => n.resting_hr_bpm,
            Self::Resp => n.resp_bpm,
            Self::SkinTemp => n.skin_temp_c,
        }
    }
}
...
impl Default for NightGate {
    fn default() -> Self {
        Self {
            min_sleep_secs: MIN_SLEEP_SECS,
            worn_skin_temp_c: WORN_SKIN_TEMP_C,
            min_quality: None,
        }
    }
}
...
impl NightVerdict {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::Valid => "valid",
            Self::SleepTooShort => "sleep_too_short",
            Self::SkinTempImplausible => "skin_temp_implausible",
            Self::RestingHrImplausible => "resting_hr_implausible",
            Self::HrvImplausible => "hrv_implausible",
            Self::RespImplausible => "resp_implausible",
...
pub fn night_verdict(n: &NightChannels, gate: &NightGate) -> NightVerdict {
    ...
    if out_of_band(n.hrv_ms, hrv.min_val, hrv.max_val) {
...
pub fn fold_history_nights(
    nights: &[NightChannels],
    metric: NightMetric,
    gate: &NightGate,
) -> BaselineState {
    let cfg = metric.cfg();
    let values: Vec<Option<f64>> = nights
        .iter()
        .map(|n| {
            if night_verdict(n, gate).valid() {
                metric.value(n)
            } else {
                None
            }
...
impl NightSkinTemp {
    ...
    pub fn coverage(&self) -> f64 {
        if self.n_total == 0 {
            ...
            self.n_kept as f64 / self.n_total as f64
        }
    }
}
...
pub fn night_skin_temp(samples: &[(f64, f64)], min_conf: f64) -> Option<NightSkinTemp> {
    ...
    Some(NightSkinTemp {
        median_c: crate::stats::median(&kept),
        max_c,
        n_kept: kept.len(),
        n_total: samples.len(),
    })
...
mod tests {
    ...
    fn fold_reproduces_the_ewma_centre_a_z_score_is_measured_against() {
        ...
        assert_eq!(
            (s.baseline, s.spread, s.n_valid),
            (50.0, cfg.floor_spread, 1)
        );
        ...
        assert!(
            (s.baseline - young).abs() < 1e-12,
            "after 7 young folds got {}, want {young}",
            s.baseline
        );
        ...
        assert!(
            (s.baseline - steady).abs() < 1e-12,
            "after 20 folds got {}, want {steady}",
            s.baseline
        );
    ...
    fn a_do_nothing_centre_fails_the_fold() {
        let cfg = MetricCfg::hrv();
        let converging: Vec<Option<f64>> = std::iter::once(Some(50.0))
            .chain(std::iter::repeat_n(Some(45.0), 20))
            .collect();
        let swinging: Vec<Option<f64>> = (0..20)
            .map(|i| Some(if i % 2 == 0 { 40.0 } else { 60.0 }))
            .collect();
        let with_outlier: Vec<Option<f64>> = std::iter::repeat_n(Some(45.0), 12)
            .chain(std::iter::once(Some(120.0)))
            .collect();
        ...
        for (name, f) in scorers {
            let misses = series
                .iter()
                .any(|s| (f(s) - fold_history(s, &cfg).baseline).abs() > 0.05);
            assert!(
                misses,
                "the do-nothing centre `{name}` reproduces the fold on every series"
            );
        }
    }
    ...
    fn a_hard_outlier_night_is_seen_but_never_folded() {
        ...
        assert_eq!(
            held.nights_since_update, 0,
            "the night was seen, so staleness restarts"
        );
        let folded = update(Some(before), Some(70.0), &cfg); // dev 25 is not PAST the threshold
        let lb = lambda(cfg.half_life_b);
        let winsorised = 45.0 + WINSOR_K * 5.0 * lb;
        assert!(
            (folded.baseline - winsorised).abs() < 1e-12,
            "got {}, want {winsorised}",
            folded.baseline
        );
        assert_eq!(folded.n_valid, 11);
        assert!(
            folded.baseline < 45.0 + 25.0 * lb,
            "winsorising is what caps the move"
        );
    }
    ...
    fn winsorised_fold_lifts_the_spread_off_its_floor() {
        ...
        assert!(
            (s.spread - want).abs() < 1e-12,
            "got {}, want {want}",
            s.spread
        );
        assert!(
            s.spread > cfg.floor_spread,
            "spread stayed on its floor at {}",
            s.spread
        );
        // A steady series never leaves the floor, so a gate built on one cannot see this limb at all.
        assert_eq!(
            fold_history(&vec![Some(45.0); 21], &cfg).spread,
            cfg.floor_spread
        );
    }
    ...
    fn a_young_baseline_has_no_outlier_guard() {
        let cfg = MetricCfg::hrv();
        let young = update(
            Some(seeded(45.0, 5.0, EARLY_ADAPT_NIGHTS - 1)),
            Some(200.0),
            &cfg,
        );
        ...
        assert!(
            (young.baseline - (elb * clamped + (1.0 - elb) * 45.0)).abs() < 1e-12,
            "got {}",
            young.baseline
        );
        assert_eq!(
            young.n_valid, EARLY_ADAPT_NIGHTS,
            "a young night is always folded"
        );
        assert!(
            (young.baseline - 52.736230276).abs() < 1e-9,
            "got {}",
            young.baseline
        );
        ...
        assert_eq!(
            update(
                Some(seeded(45.0, 5.0, EARLY_ADAPT_NIGHTS)),
                Some(200.0),
                &cfg
            )
            .baseline,
    ...
    fn the_status_lifecycle_reaches_stale_and_recovers() {
        ...
        for _ in 1..MIN_NIGHTS_SEED {
            s = update(Some(s), Some(45.0), &cfg);
        }
        assert_eq!(
            (s.n_valid, s.status),
            (MIN_NIGHTS_SEED, BaselineStatus::Provisional)
        );
        for _ in MIN_NIGHTS_SEED..MIN_NIGHTS_TRUST {
            s = update(Some(s), Some(45.0), &cfg);
        }
        assert_eq!(
            (s.n_valid, s.status),
            (MIN_NIGHTS_TRUST, BaselineStatus::Trusted)
        );
        // STALE_DAYS missed nights are still Trusted; the next one tips it.
        for _ in 0..STALE_DAYS {
            s = update(Some(s), None, &cfg);
        }
        assert_eq!(
            (s.nights_since_update, s.status),
            (STALE_DAYS, BaselineStatus::Trusted)
        );
        s = update(Some(s), None, &cfg);
        assert_eq!(
            (s.nights_since_update, s.status),
            (STALE_DAYS + 1, BaselineStatus::Stale)
        );
        assert_eq!(
            (s.n_valid, s.baseline),
            (MIN_NIGHTS_TRUST, 45.0),
            "a stale baseline keeps its nights"
        );
        s = update(Some(s), Some(45.0), &cfg);
        assert_eq!(
            (s.nights_since_update, s.status),
            (0, BaselineStatus::Trusted)
        );
        // Under MIN_NIGHTS_SEED there is nothing to go stale.
        let unseeded: Vec<Option<f64>> = std::iter::repeat_n(Some(45.0), 2)
            .chain(std::iter::repeat_n(None, 40))
            .collect();
    ...
    fn a_plausible_night_is_valid() {
        assert_eq!(
            night_verdict(&good_night(), &NightGate::default()),
            NightVerdict::Valid
        );
    }
    ...
    fn missing_channels_never_reject() {
        ...
        assert_eq!(
            night_verdict(&n, &NightGate::default()),
            NightVerdict::Valid
        );
    }
    ...
    fn nightstand_night_rejects_every_metric() {
        ...
        assert_eq!(
            night_verdict(&n, &NightGate::default()),
            NightVerdict::SkinTempImplausible
        );
        ...
        assert!(
            MetricCfg::skin_temp().min_val <= 22.4,
            "the storage band admits an off-wrist night"
        );
    }
    ...
    fn conjunction_changes_the_hrv_baseline_a_per_metric_gate_would_fold() {
        ...
        assert_eq!(
            new.n_valid, 9,
            "the conjunction skips the three nightstand nights"
        );
        assert!(
            new.baseline > old.baseline + 1.0,
            "old {} was dragged down by nights the strap was not worn, new {}",
            old.baseline,
            new.baseline
        );
        assert_eq!(
            night_verdicts(&nights, &NightGate::default())
                .iter()
                .filter(|v| **v == NightVerdict::SkinTempImplausible)
                .count(),
            3
        );
    }
    ...
    fn rejected_night_skip_and_holds_rather_than_disappearing() {
        ...
        assert_eq!(s.n_valid, 5);
        assert_eq!(
            s.nights_since_update, 1,
            "staleness must still advance on a rejected night"
        );
    }
    ...
    fn each_channel_can_reject_alone() {
        ...
        check(
            &|n| n.total_sleep_secs = Some(3.9 * 3600.0),
            NightVerdict::SleepTooShort,
        );
        check(
            &|n| n.skin_temp_max_c = Some(41.0),
            NightVerdict::SkinTempImplausible,
        );
        check(
            &|n| n.resting_hr_bpm = Some(180.0),
            NightVerdict::RestingHrImplausible,
        );
        check(&|n| n.hrv_ms = Some(400.0), NightVerdict::HrvImplausible);
        check(&|n| n.resp_bpm = Some(60.0), NightVerdict::RespImplausible);
        check(
            &|n| n.skin_temp_c = Some(f64::NAN),
            NightVerdict::SkinTempImplausible,
        );
    }
    ...
    const TEMP_NIGHTS: &[TempNight] = &[
        TempNight {
            what: "a low-confidence sample is dropped before the median",
        ...
        TempNight {
            what: "an even count takes the mean of the two middles",
        ...
        TempNight {
            what: "one hot window moves the max but not the centre",
            samples: &[
                (33.0, 0.9),
                (33.125, 0.9),
                (33.25, 0.9),
        ...
        TempNight {
            what: "the samples are sorted before the middle is taken",
        ...
        TempNight {
            what: "confidence exactly at the floor is kept",
        ...
        TempNight {
            what: "a non-finite sample is dropped however confident",
    ...
    fn night_skin_temp_medians_the_confident_samples_of_every_night() {
        for n in TEMP_NIGHTS {
            ...
            assert!(
                (s.median_c - n.median_c).abs() < 1e-12,
                "{}: centre {}",
                n.what,
                s.median_c
            );
            assert!(
                (s.max_c - n.max_c).abs() < 1e-12,
                "{}: max {}",
                n.what,
                s.max_c
            );
    ...
    fn a_do_nothing_night_reducer_fails_the_skin_temp_nights() {
        ...
        let first_confident = |s: &[(f64, f64)]| {
            s.iter()
                .find(|(v, c)| v.is_finite() && *c >= MIN_CONF)
                .map(|(v, _)| *v)
                .unwrap()
        };
        let scorers: [Scorer<&[(f64, f64)]>; 5] = [
            ...
            (
                "median without the confidence filter",
                &median_ignoring_confidence,
            ),
            ("the first confident sample", &first_confident),
        ];
        for (name, f) in scorers {
            let misses = TEMP_NIGHTS
                .iter()
                .any(|n| (f(n.samples) - n.median_c).abs() > 1e-9);
            assert!(
                misses,
                "the do-nothing reducer `{name}` reproduces every night's centre"
            );
        }
    }
    ...
    fn nightly_skin_temp_medians_drive_the_skin_temp_baseline() {
        ...
        let nights: Vec<NightChannels> = centres
            .iter()
            .map(|&c| {
                let t = night_skin_temp(&samples(c), MIN_CONF).unwrap();
                assert!(
                    (t.median_c - c).abs() < 1e-12,
                    "the reducer must recover the planted centre"
                );
                NightChannels {
                    skin_temp_c: Some(t.median_c),
                    skin_temp_max_c: Some(t.max_c),
                    total_sleep_secs: Some(7.0 * 3600.0),
                    quality: Some(t.coverage()),
                    ..Default::default()
                }
            })
            .collect();
        ...
        let (elb, lb) = (
            lambda(EARLY_HALF_LIFE_B),
            lambda(MetricCfg::skin_temp().half_life_b),
        );
        let mut want = centres[0];
        for (i, &c) in centres.iter().enumerate().skip(1) {
            let rate = if i < EARLY_ADAPT_NIGHTS as usize {
                elb
            } else {
                lb
            };
        ...
        let flat: Vec<NightChannels> = nights
            .iter()
            .map(|n| NightChannels {
                skin_temp_c: Some(33.4),
                ..*n
            })
            .collect();
        let f = fold_history_nights(&flat, NightMetric::SkinTemp, &NightGate::default());
        assert!(
            (f.baseline - s.baseline).abs() > 0.1,
            "a constant nightly centre reaches the same baseline: {} vs {}",
            f.baseline,
            s.baseline
        );
    }
