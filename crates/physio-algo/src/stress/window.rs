...
use super::{HIGH_BAND_FLOOR, SD_FLOOR, StressBand, band_of, mean_opt, population_std, squash};
...
fn calm_reference(xs: &[f64], calm_is_low: bool) -> Option<f64> {
    ...
    if xs.len() < CALM_QUARTILE_MIN_COUNT {
        return mean_opt(xs);
    }
    let mut s = xs.to_vec();
...
mod tests {
    ...
    const REAL_DAY_MOTION_G: [f64; 24] = [
        0.021853, 0.006737, 0.010248, 0.009181, 0.008856, 0.008557, 0.015612, 0.011207, 0.016108,
        0.015935, 0.093877, 0.068195, 0.111046, 0.085727, 0.142547, 0.039923, 0.129114, 0.119880,
    ...
    fn real_day_reproduces_the_shipped_scores() {
        ...
        assert!(
            day.iter().all(|h| h.motion_g.is_some()),
            "the pinned day lost its motion channel"
        );
    ...
    fn the_real_day_never_reaches_the_activity_gate() {
        ...
        assert!(
            busiest < ACTIVITY_GATE_G,
            "{busiest} g would suppress an hour of the golden day"
        );
        ...
        let at = |g: f64| {
            windowed_stress(
                ...
                StressWindowCfg {
                    bucket_seconds: HOUR_SECONDS,
                    sleep_spans: &waking_window_as_spans(),
                    activity_gate_g: Some(g),
                    sustained_buckets: SUSTAINED_BUCKETS,
                },
            )
        };
        ...
        assert_eq!(
            lowered.mean.unwrap().to_bits(),
            2.1705370801496633f64.to_bits()
        );
        assert_eq!(
            (
                lowered.low_minutes,
                lowered.medium_minutes,
                lowered.high_minutes
            ),
            (60, 240, 600)
        );
        let raised = at(0.165);
        ...
        assert_eq!(
            raised.mean.unwrap().to_bits(),
            2.1982490363146434f64.to_bits()
        );
    }
    ...
    fn real_day_high_share_matches_its_band_minutes() {
        ...
        assert_eq!(
            r.high_share_pct().unwrap().to_bits(),
            (660.0f64 * 100.0 / 960.0).to_bits()
        );
    }
    ...
    fn a_flat_day_cannot_reproduce_the_golden_day() {
        ...
        assert!(
            null.buckets
                .iter()
                .all(|b| b.stress.to_bits() == squash(0.0).to_bits())
        );
        assert_eq!(null.mean.unwrap().to_bits(), squash(0.0).to_bits());
        assert_eq!(
            (null.low_minutes, null.medium_minutes, null.high_minutes),
            (0, 960, 0)
        );
    ...
    fn the_golden_day_scored_as_a_night_reproduces_its_own_shipped_scores() {
        ...
        let hr_mean =
            mean_opt(&night.iter().filter_map(|h| h.mean_hr).collect::<Vec<_>>()).unwrap();
        let rmssd_mean =
            mean_opt(&night.iter().filter_map(|h| h.rmssd).collect::<Vec<_>>()).unwrap();
        let flat: Vec<HourPoint> = night
            .iter()
            .map(|h| HourPoint {
                mean_hr: Some(hr_mean),
                rmssd: Some(rmssd_mean),
                ..*h
            })
            .collect();
    ...
    fn a_non_clock_span_set_moves_the_golden_day() {
        ...
        assert_eq!(
            r.suppressed_count(Suppression::Asleep),
            7,
            "00:00–06:00 go, 06:00 by 40 min"
        );
        assert_eq!(
            r.buckets.iter().map(|b| b.hour).collect::<Vec<_>>(),
            (7..24).collect::<Vec<_>>()
        );
        ...
        assert!(!r.sustained_high);
        assert_eq!(
            r.buckets.last().unwrap().stress.to_bits(),
            1.9951226376307365f64.to_bits()
        );
    }
    ...
    fn flat_day_scores_neutral() {
        ...
        assert!(
            (r.mean.unwrap() - 1.5).abs() < 0.1,
            "flat day → neutral, got {}",
            r.mean.unwrap()
        );
        assert!(!r.sustained_high);
    }
    ...
    fn spiky_afternoon_scores_high_and_sustained() {
        ...
        assert!(
            r.sustained_high,
            "3+ spiky hours should trigger sustained high"
        );
    ...
    fn the_calm_reference_switches_from_the_mean_to_the_quartile_at_four_values() {
        ...
        assert_ne!(
            calm_reference(&three, true).unwrap(),
            percentile(&three, 0.25)
        );
        assert_ne!(
            calm_reference(&three, false).unwrap(),
            percentile(&three, 0.75)
        );
        ...
        assert_eq!(
            calm_reference(&four, true).unwrap(),
            percentile(&four, 0.25)
        );
        assert_eq!(
            calm_reference(&four, false).unwrap(),
            percentile(&four, 0.75)
        );
    ...
    fn a_three_bucket_nap_is_scored_against_its_mean_not_its_quartile() {
        ...
        let nap: Vec<HourPoint> = hr
            .iter()
            .enumerate()
            .map(|(i, &h)| point(13 + i as i32, Some(h), Some(40.0)))
            .collect();
        ...
        for (b, &h) in r.buckets.iter().zip(hr.iter()) {
            assert_eq!(
                b.stress.to_bits(),
                squash((h - mean) / sd).to_bits(),
                "hour {}",
                b.hour
            );
            assert_ne!(
                b.stress.to_bits(),
                squash((h - percentile(&hr, 0.25)) / sd).to_bits()
            );
        }
    ...
    fn sleep_span_suppresses_and_names_the_bucket() {
        ...
        assert!(
            r.buckets.is_empty(),
            "a bucket inside a sleep span is not scored"
        );
    ...
    fn an_afternoon_nap_is_suppressed_mid_day() {
        ...
        let spans = [
            span(14, 15),
            SpanMs {
                start_ms: DAY_START_MS + 15 * HOUR_MS,
                end_ms: DAY_START_MS + 15 * HOUR_MS + HOUR_MS / 2,
            },
        ];
        ...
        assert_eq!(
            r.suppressed_count(Suppression::Asleep),
            2,
            "14:00 and the partly-overlapped 15:00"
        );
    ...
    fn a_bucket_touching_a_span_edge_is_kept() {
        ...
        let points = vec![
            point(5, Some(70.0), Some(40.0)),
            point(6, Some(70.0), Some(40.0)),
        ];
    ...
    fn night_derivation_scores_inside_its_own_span() {
        ...
        assert!(
            daytime_stress(&night, &waking_window_as_spans())
                .buckets
                .is_empty()
        );
        let r = sleep_stress(&night);
    ...
    fn the_activity_gate_is_inclusive_at_its_edge() {
        let at = vec![HourPoint {
            motion_g: Some(ACTIVITY_GATE_G),
            ..point(10, Some(70.0), Some(40.0))
        }];
        let below = vec![HourPoint {
            motion_g: Some(ACTIVITY_GATE_G - 1e-9),
            ..point(10, Some(70.0), Some(40.0))
        }];
        assert_eq!(
            daytime_stress(&at, &[]).suppressed_count(Suppression::Active),
            1
        );
    ...
    fn motion_wins_over_sleep_when_a_bucket_is_both() {
        let points = vec![HourPoint {
            motion_g: Some(0.5),
            ..point(2, Some(60.0), Some(50.0))
        }];
        let r = daytime_stress(&points, &waking_window_as_spans());
    ...
    fn night_crossing_midnight_keeps_every_bucket() {
        ...
        let points: Vec<HourPoint> = spans
            .iter()
            .enumerate()
            .map(|(i, &h)| HourPoint {
    ...
    fn bucket_seconds_sizes_the_band_minutes_and_the_sleep_test() {
        ...
        assert_eq!(
            r.suppressed.len(),
            1,
            "only the 10:15 bucket lies inside the span"
        );
    ...
    fn sustained_buckets_is_the_threshold_the_trailing_run_must_reach() {
        ...
        assert!(
            at(SUSTAINED_BUCKETS).buckets[..8]
                .iter()
                .all(|b| b.stress < HIGH_BAND_FLOOR)
        );
        assert!(
            at(SUSTAINED_BUCKETS).buckets[8..]
                .iter()
        ...
        assert!(
            at(run).sustained_high,
            "a run of {run} must satisfy a threshold of {run}"
        );
        assert!(
            !at(run + 1).sustained_high,
            "a run of {run} must not satisfy {}",
            run + 1
        );
        ...
        assert_eq!(
            ended.high_minutes,
            4 * 60,
            "the four high hours are still in the band tally"
        );
    }
}
