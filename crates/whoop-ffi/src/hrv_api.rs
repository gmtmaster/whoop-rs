...
#[uniffi::export]
pub fn ppg_hr_aggregate(estimates: Vec<PpgEstimate>, bucket_secs: i64) -> Vec<PpgEstimate> {
    let e: Vec<ppg_hr::Estimate> =
        estimates.into_iter().map(|p| ppg_hr::Estimate { ts: p.ts, bpm: p.bpm, conf: p.conf }).collect();
    ppg_hr::aggregate(&e, bucket_secs)
        .into_iter()
        .map(|e| PpgEstimate { ts: e.ts, bpm: e.bpm, conf: e.conf })
        .collect()
}
...
#[uniffi::export]
pub fn ppg_signal_check(estimates: Vec<PpgEstimate>, start: i64, end: i64) -> PpgSignalCheck {
    let e: Vec<ppg_hr::Estimate> =
        estimates.into_iter().map(|p| ppg_hr::Estimate { ts: p.ts, bpm: p.bpm, conf: p.conf }).collect();
    match ppg_hr::signal_check(&e, start, end) {
        ppg_hr::SignalCheck::Poor => PpgSignalCheck::Poor,
        ppg_hr::SignalCheck::Fair => PpgSignalCheck::Fair,
        ppg_hr::SignalCheck::Good => PpgSignalCheck::Good,
    }
}
...
#[uniffi::export]
pub fn ppg_check_cfg() -> PpgCheckCfgInfo {
    PpgCheckCfgInfo {
        min_confidence: ppg_hr::MIN_CONFIDENCE,
        good_confidence: ppg_hr::GOOD_CONFIDENCE,
        fair_fraction: ppg_hr::SIGNAL_CHECK_FAIR_FRACTION,
        good_fraction: ppg_hr::SIGNAL_CHECK_GOOD_FRACTION,
    }
}
...
#[uniffi::export]
pub fn ppg_hr_derate_poor(estimates: Vec<PpgEstimate>, poor_secs: Vec<i64>) -> Vec<PpgEstimate> {
    let e: Vec<ppg_hr::Estimate> =
        estimates.into_iter().map(|p| ppg_hr::Estimate { ts: p.ts, bpm: p.bpm, conf: p.conf }).collect();
    ppg_hr::derate_poor_seconds(&e, &poor_secs.into_iter().collect())
        .into_iter()
        .map(|e| PpgEstimate { ts: e.ts, bpm: e.bpm, conf: e.conf })
        .collect()
}
...
#[uniffi::export]
pub fn rr_beats_trusted(optical_signal_poor: Option<bool>) -> bool {
    physio_algo::hrv::rr_trusted(optical_signal_poor)
}
