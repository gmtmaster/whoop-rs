...
| `physio-algo` | Every derived metric, pure and sans-IO: the whole sleep pipeline (detect / stage / refine / main-night), versioned nightly physiology (dynamic RHR plus primary final-SWS HRV with a quality-identical six-minute short-SWS fallback), recovery, strain, respiration, SpO2, calories, workout, steps, IMU features, the ages, vital banding, the hydration goal and the calibration timeline. `algorithms.md` is its per-formula record | ~10680 LOC | 308 + 4 `#[ignore]`d |
